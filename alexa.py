...
def start( place, time, zipcode ):
    ...
    if len( names ) == 1:
        return statement( render_template( "response", name = names[ 0 ][ 0 ], rating = names[ 0 ][ 1 ] ) );
    if len( names ) == 2:
        return statement( render_template( "response2", name = names[ 0 ][ 0 ], rating = names[ 0 ][ 1 ], name2 = names[ 1 ][ 0 ], rating2 = names[ 1 ][ 1 ] ) );
    return statement( render_template( "response3", name = names[ 0 ][ 0 ], rating = names[ 0 ][ 1 ], name2 = names[ 1 ][ 0 ], rating2 = names[ 1 ][ 1 ], name3 = names[ 2 ][ 0 ], rating3 = names[ 2 ][ 1 ] ) );
